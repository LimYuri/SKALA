...
## 09-15 ~ 09-17 · AI 웹 서비스 설계 Mini-project (7반, 개인 과제)
무대로그(StageLog) — 밴드 멤버들의 합주 가능 시간을 조율하고, 공연 셋리스트·큐시트를 준비하고, 공연 후 사진·영상·회고를 한곳에 남기는 밴드 일정·기록 관리 서비스 기획. 회원·밴드 관리, 일정 조율, 셋리스트·큐시트, 성장 기록·포트폴리오 4개 영역으로 기능을 나누고 기획서부터 화면설계, API 명세, ERD까지 설계했다.

- `무대로그_StageLog/PG7반_P231_임유리_무대로그_개요.pdf` — 서비스 기획서 & 화면설계서 (기획 배경, 페인 포인트, 핵심 기능, 서비스 흐름, 화면 설계)
- `무대로그_StageLog/PG7반_P231_임유리_무대로그_API.pdf`, `..._API.yml` — API 명세서 (OpenAPI)
- `무대로그_StageLog/PG7반_P231_임유리_무대로그_DB.pdf`, `..._DB.dbml` — ERD (DBML)
